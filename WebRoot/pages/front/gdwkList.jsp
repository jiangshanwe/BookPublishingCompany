		...
		<div id="central">
			...
			<div class="central_right">
				<div class="right_title">
					...
					<p style="float: right; padding-right: 12px;">
						<form method="post" action="osearchBook">
							<input type="text" value="<s:property value='%{#session.obkey}'/>"
								name="obkey"
				...
				<div id="right_messageBook">
					<ul>
						<s:if test="#session.gdwkList.size==0">
							<p style="margin-left: 83px; margin-top: 46px;">
								暂无图书信息
							</p>
						</s:if>
						<s:iterator value="#session.gdwkList" id="bl" status="st">
							...
							<a href="fgdwkDetail?id=<s:property value='%{#bl.id}' />"
								target="_blank"> <img alt="暂无图书照片"
									src='bookPic/gdwk/<s:property value="%{#bl.tupian1}"/>'>
							</a>
							<p>
								<a href="fgdwkDetail?id=<s:property value='%{#bl.id}' />" target="_blank"><s:property value="#bl.shuming" />
								</a>
								<br>
				...
				<div class="pagelist">
					<ul>
						<s:if test="#session.page!=1">
							<li>
								<a href="fgdwkList?page=1">首页</a>
							</li>

							<li>
								<a href="fgdwkSyy">上页</a>
							</li>
						</s:if>
						<s:if test="#session.page< #session.pageCount">
							<li>
								<a href="fgdwkXyy">下页</a>
							</li>
							<li>
								<a
									href="fgdwkList?page=<s:property value='%{#session.pageCount}'/>">尾页</a>
							</li>
						</s:if>
						<li class="pageinfo">
							第&nbsp;
