...
<html>
	<head>
		...
		<title>My JSP 'zzjyShowall.jsp' starting page</title>
	...
	<body>
		<form action="zzjyTianjia">
			<s:submit value="添加新的中职教育图书" align="left"></s:submit>
		</form>
		<form action="zzjyRePaixu">
		...
		<div>
			<table border="1" align="center">
				...
				<s:iterator value="#session.allZzjys" id="als" status="L">
					<tr align="center" height="49.6">
						<td>
							<s:property value="#als.paixu" />
						</td>
						<td width="165">
							<s:url id="url" action="zzjyYulan">
						...
						<td width="120" height="90">
							<a href='bookPic/zzjy/<s:property value="%{#als.tupian1}"/>'
								target="_blank"><img width="90" height="120" alt="暂无图书照片"
									src='bookPic/zzjy/<s:property value="%{#als.tupian1}"/>'>
							</a>
						</td>
						<td width="120">
						...
						<td width="50">
							<s:form action="zzjyMoveUp">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↑"></s:submit>
							</s:form>
							<s:form action="zzjyMoveDown">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↓"></s:submit>
							</s:form>
						</td>
						<td width="80">
							<s:form action="zzjyBianji">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="编辑"></s:submit>
							</s:form>
						</td>
						<td width="60">
							<s:form action="zzjyShanchu">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
					...
					<td colspan="12" height="26" align="center">
						...
						<s:bean name="org.apache.struts2.util.Counter" var="i">
							...
							<s:iterator>
								...
								<s:else>
									<a
										href="zzjyShowall?page=<s:property value='#i.current - 1' />">
										<s:property value="#i.current - 1" /> </a>&nbsp;
								</s:else>
							</s:iterator>
						</s:bean>
						<s:if test="#session.pageCount != page">
							<a href="zzjyXyy">下一页</a>
						</s:if>
					</td>
				</tr>
			</table>
