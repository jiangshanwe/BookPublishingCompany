...
<html>
	<head>
		...
		<title>My JSP 'gdlgShowall.jsp' starting page</title>
	...
	<body>
		<form action="gdlgTianjia">
			<s:submit value="添加新的高等理工图书" align="left"></s:submit>
		</form>
		<form action="gdlgRePaixu">
		...
		<div>
			<table border="1" align="center">
				...
				<s:iterator value="#session.allGdlgs" id="als" status="L">
					<tr align="center" height="49.6">
						<td>
							<s:property value="#als.paixu" />
						</td>
						<td width="165">
							<s:url id="url" action="gdlgYulan">
						...
						<td width="120" height="90">
							<a href='bookPic/gdlg/<s:property value="%{#als.tupian1}"/>'
								target="_blank"><img width="90" height="120" alt="暂无图书照片"
									src='bookPic/gdlg/<s:property value="%{#als.tupian1}"/>'>
							</a>
						</td>
						<td width="120">
						...
						<td width="50">
							<s:form action="gdlgMoveUp">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↑"></s:submit>
							</s:form>
							<s:form action="gdlgMoveDown">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↓"></s:submit>
							</s:form>
						</td>
						<td width="80">
							<s:form action="gdlgBianji">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="编辑"></s:submit>
							</s:form>
						</td>
						<td width="60">
							<s:form action="gdlgShanchu">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
					...
					<td colspan="12" height="26" align="center">
						...
						<s:bean name="org.apache.struts2.util.Counter" var="i">
							...
							<s:iterator>
								...
								<s:else>
									<a
										href="gdlgShowall?page=<s:property value='#i.current - 1' />">
										<s:property value="#i.current - 1" /> </a>&nbsp;
								</s:else>
							</s:iterator>
						</s:bean>
						<s:if test="#session.pageCount != page">
							<a href="gdlgXyy">下一页</a>
						</s:if>
					</td>
				</tr>
			</table>
