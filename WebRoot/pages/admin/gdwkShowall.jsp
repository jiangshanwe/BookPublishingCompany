...
<html>
	<head>
		...
		<title>My JSP 'gdwkShowall.jsp' starting page</title>
	...
	<body>
		<form action="gdwkTianjia">
			<s:submit value="添加新的高等文科图书" align="left"></s:submit>
		</form>
		<form action="gdwkRePaixu">
		...
		<div>
			<table border="1" align="center">
				...
				<s:iterator value="#session.allGdwks" id="als" status="L">
					<tr align="center" height="49.6">
						<td>
							<s:property value="#als.paixu" />
						</td>
						<td width="165">
							<s:url id="url" action="gdwkYulan">
						...
						<td width="120" height="90">
							<a href='bookPic/gdwk/<s:property value="%{#als.tupian1}"/>'
								target="_blank"><img width="90" height="120" alt="暂无图书照片"
									src='bookPic/gdwk/<s:property value="%{#als.tupian1}"/>'>
							</a>
						</td>
						<td width="120">
						...
						<td width="50">
							<s:form action="gdwkMoveUp">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↑"></s:submit>
							</s:form>
							<s:form action="gdwkMoveDown">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↓"></s:submit>
							</s:form>
						</td>
						<td width="80">
							<s:form action="gdwkBianji">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="编辑"></s:submit>
							</s:form>
						</td>
						<td width="60">
							<s:form action="gdwkShanchu">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
					...
					<td colspan="12" height="26" align="center">
						...
						<s:bean name="org.apache.struts2.util.Counter" var="i">
							...
							<s:iterator>
								...
								<s:else>
									<a
										href="gdwkShowall?page=<s:property value='#i.current - 1' />">
										<s:property value="#i.current - 1" /> </a>&nbsp;
								</s:else>
							</s:iterator>
						</s:bean>
						<s:if test="#session.pageCount != page">
							<a href="gdwkXyy">下一页</a>
						</s:if>
					</td>
				</tr>
			</table>
