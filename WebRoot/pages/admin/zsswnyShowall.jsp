...
<html>
	<head>
		...
		<title>My JSP 'zsswnyShowall.jsp' starting page</title>
	...
	<body>
		<form action="zsswnyTianjia">
			<s:submit value="添加新的产品展示-生物农医" align="left"></s:submit>
		</form>
		<form action="zsswnyRePaixu">
		...
		<div>
			<table border="1" align="center">
				...
				<s:iterator value="#session.allZsswnys" id="als" status="L">
					<tr align="center">
						<td>
						...
						<td>
							<a href='<s:property value="%{#als.lianjie}"/>'
								target="_blank"><img width="90" height="120" alt="暂无图书照片"
									src='fujian/zsswny/<s:property value="%{#als.tupian}"/>'>
							</a>
						</td>
						<td>
						...
						<td width="150">
							<s:form action="zsswnyMoveUp">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↑"></s:submit>
							</s:form>
							<s:form action="zsswnyMoveDown">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↓"></s:submit>
							</s:form>
						</td>
						<td width="120">
							<s:form action="zsswnyBianji">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="编辑"></s:submit>
							</s:form>
						</td>
						<td width="60">
							<s:form action="zsswnyShanchu">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="删除" onclick="return confirm('确认删除此条下载中展示图书？')"></s:submit>
							</s:form>
						</td>
					</tr>
				</s:iterator>
					...
					<td colspan="9" height="26" align="center">
						...
						<s:bean name="org.apache.struts2.util.Counter" var="i">
							...
							<s:iterator>
								...
								<s:else>
									<a
										href="zsswnyShowall?page=<s:property value='#i.current - 1' />">
										<s:property value="#i.current - 1" /> </a>&nbsp;
								</s:else>
							</s:iterator>
						</s:bean>
						<s:if test="#session.pageCount != page">
							<a href="zsswnyXyy">下一页</a>
						</s:if>
					</td>
				</tr>
			</table>
