...
<html>
	<head>
		...
		<title>My JSP 'hydtShowall.jsp' starting page</title>
	...
	<body>
		<form action="hydtTianjia">
			<s:submit value="添加新的行业动态" align="left"></s:submit>
		</form>
		<form action="hydtRePaixu">
		...
		<div>
			<table border="1" align="center">
				...
				<s:iterator value="#session.allHydts" id="als" status="L">
					<tr align="center">
						<td>
							<s:property value="#als.paixu" />
						</td>
						<td>
							<s:url id="url" action="hydtYulan">
						...
						<td width="150">
							<s:form action="hydtMoveUp">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↑"></s:submit>
							</s:form>
							<s:form action="hydtMoveDown">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↓"></s:submit>
							</s:form>
						</td>
						<td width="120">
							<s:form action="hydtBianji">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="编辑"></s:submit>
							</s:form>
						</td>
						<td width="60">
							<s:form action="hydtShanchu">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="删除" onclick="return confirm('确认删除此条行业动态？')"></s:submit>
							</s:form>
						</td>
					</tr>
				</s:iterator>
					...
					<td colspan="8" height="26" align="center">
						...
						<s:bean name="org.apache.struts2.util.Counter" var="i">
							...
							<s:iterator>
								...
								<s:else>
									<a
										href="hydtShowall?page=<s:property value='#i.current - 1' />">
										<s:property value="#i.current - 1" /> </a>&nbsp;
								</s:else>
							</s:iterator>
						</s:bean>
						<s:if test="#session.pageCount != page">
							<a href="hydtXyy">下一页</a>
						</s:if>
					</td>
				</tr>
			</table>
