...
<html>
	<head>
		...
		<title>My JSP 'zszzjyShowall.jsp' starting page</title>
	...
	<body>
		<form action="zszzjyTianjia">
			<s:submit value="添加新的产品展示-中职教育" align="left"></s:submit>
		</form>
		<form action="zszzjyRePaixu">
		...
		<div>
			<table border="1" align="center">
				...
				<s:iterator value="#session.allZszzjys" id="als" status="L">
					<tr align="center">
						<td>
						...
						<td>
							<a href='<s:property value="%{#als.lianjie}"/>'
								target="_blank"><img width="90" height="120" alt="暂无图书照片"
									src='fujian/zszzjy/<s:property value="%{#als.tupian}"/>'>
							</a>
						</td>
						<td>
						...
						<td width="150">
							<s:form action="zszzjyMoveUp">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↑"></s:submit>
							</s:form>
							<s:form action="zszzjyMoveDown">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↓"></s:submit>
							</s:form>
						</td>
						<td width="120">
							<s:form action="zszzjyBianji">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="编辑"></s:submit>
							</s:form>
						</td>
						<td width="60">
							<s:form action="zszzjyShanchu">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
					...
					<td colspan="9" height="26" align="center">
						...
						<s:bean name="org.apache.struts2.util.Counter" var="i">
							...
							<s:iterator>
								...
								<s:else>
									<a
										href="zszzjyShowall?page=<s:property value='#i.current - 1' />">
										<s:property value="#i.current - 1" /> </a>&nbsp;
								</s:else>
							</s:iterator>
						</s:bean>
						<s:if test="#session.pageCount != page">
							<a href="zszzjyXyy">下一页</a>
						</s:if>
					</td>
				</tr>
			</table>
