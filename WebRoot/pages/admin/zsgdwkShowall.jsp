...
<html>
	<head>
		...
		<title>My JSP 'zsgdwkShowall.jsp' starting page</title>
	...
	<body>
		<form action="zsgdwkTianjia">
			<s:submit value="添加新的产品展示-高等文科" align="left"></s:submit>
		</form>
		<form action="zsgdwkRePaixu">
		...
		<div>
			<table border="1" align="center">
				...
				<s:iterator value="#session.allZsgdwks" id="als" status="L">
					<tr align="center">
						<td>
						...
						<td>
							<a href='<s:property value="%{#als.lianjie}"/>'
								target="_blank"><img width="90" height="120" alt="暂无图书照片"
									src='fujian/zsgdwk/<s:property value="%{#als.tupian}"/>'>
							</a>
						</td>
						<td>
						...
						<td width="150">
							<s:form action="zsgdwkMoveUp">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↑"></s:submit>
							</s:form>
							<s:form action="zsgdwkMoveDown">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↓"></s:submit>
							</s:form>
						</td>
						<td width="120">
							<s:form action="zsgdwkBianji">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="编辑"></s:submit>
							</s:form>
						</td>
						<td width="60">
							<s:form action="zsgdwkShanchu">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
					...
					<td colspan="9" height="26" align="center">
						...
						<s:bean name="org.apache.struts2.util.Counter" var="i">
							...
							<s:iterator>
								...
								<s:else>
									<a
										href="zsgdwkShowall?page=<s:property value='#i.current - 1' />">
										<s:property value="#i.current - 1" /> </a>&nbsp;
								</s:else>
							</s:iterator>
						</s:bean>
						<s:if test="#session.pageCount != page">
							<a href="zsgdwkXyy">下一页</a>
						</s:if>
					</td>
				</tr>
			</table>
