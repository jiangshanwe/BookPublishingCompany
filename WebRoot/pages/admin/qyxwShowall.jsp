...
<html>
	<head>
		...
		<title>My JSP 'qyxwShowall.jsp' starting page</title>
	...
	<body>
		<form action="qyxwTianjia">
			<s:submit value="添加新的企业新闻" align="left"></s:submit>
		</form>
		<form action="qyxwRePaixu">
		...
		<div>
			<table border="1" align="center">
				...
				<s:iterator value="#session.allQyxws" id="als" status="L">
					<tr align="center">
						<td>
							<s:property value="#als.paixu" />
						</td>
						<td>
							<s:url id="url" action="qyxwYulan">
						...
						<td width="150">
							<s:form action="qyxwMoveUp">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↑"></s:submit>
							</s:form>
							<s:form action="qyxwMoveDown">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↓"></s:submit>
							</s:form>
						</td>
						<td width="120">
							<s:form action="qyxwBianji">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="编辑"></s:submit>
							</s:form>
						</td>
						<td width="60">
							<s:form action="qyxwShanchu">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="删除" onclick="return confirm('确认删除此条企业新闻？')"></s:submit>
							</s:form>
						</td>
					</tr>
				</s:iterator>
					...
					<td colspan="8" height="26" align="center">
						...
						<s:bean name="org.apache.struts2.util.Counter" var="i">
							...
							<s:iterator>
								...
								<s:else>
									<a
										href="qyxwShowall?page=<s:property value='#i.current - 1' />">
										<s:property value="#i.current - 1" /> </a>&nbsp;
								</s:else>
							</s:iterator>
						</s:bean>
						<s:if test="#session.pageCount != page">
							<a href="qyxwXyy">下一页</a>
						</s:if>
					</td>
				</tr>
			</table>
