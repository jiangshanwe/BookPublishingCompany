...
<html>
	...
	<body>
		<form action="swnyTianjia">
			<s:submit value="添加新的生物农医图书" align="left"></s:submit>
		</form>
		<form action="swnyRePaixu">
		...
		<div>
			<table border="1" align="center">
				...
				<s:iterator value="#session.allSwnys" id="als" status="L">
					<tr align="center" height="49.6">
						<td>
							<s:property value="#als.paixu" />
						</td>
						<td width="165">
							<s:url id="url" action="swnyYulan">
						...
						<td width="120" height="90">
							<a href='bookPic/swny/<s:property value="%{#als.tupian1}"/>'
								target="_blank"><img width="90" height="120" alt="暂无图书照片"
									src='bookPic/swny/<s:property value="%{#als.tupian1}"/>'>
							</a>
						</td>
						<td width="120">
						...
						<td width="50">
							<s:form action="swnyMoveUp">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↑"></s:submit>
							</s:form>
							<s:form action="swnyMoveDown">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↓"></s:submit>
							</s:form>
						</td>
						<td width="80">
							<s:form action="swnyBianji">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="编辑"></s:submit>
							</s:form>
						</td>
						<td width="60">
							<s:form action="swnyShanchu">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
					...
					<td colspan="12" height="26" align="center">
						...
						<s:bean name="org.apache.struts2.util.Counter" var="i">
							...
							<s:iterator>
								...
								<s:else>
									<a
										href="swnyShowall?page=<s:property value='#i.current - 1' />">
										<s:property value="#i.current - 1" /> </a>&nbsp;
								</s:else>
							</s:iterator>
						</s:bean>
						<s:if test="#session.pageCount != page">
							<a href="swnyXyy">下一页</a>
						</s:if>
					</td>
				</tr>
			</table>
