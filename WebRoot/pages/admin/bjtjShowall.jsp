...
<html>
	<head>
		...
		<title>My JSP 'bjtjShowall.jsp' starting page</title>
	...
	<body>
		<form action="bjtjTianjia">
			<s:submit value="添加新的编辑推荐" align="left"></s:submit>
		</form>
		<form action="bjtjRePaixu">
		...
		<div>
			<table border="1" align="center">
				...
				<s:iterator value="#session.allBjtjs" id="als" status="L">
					<tr align="center">
						<td>
						...
						<td width="150">
							<s:form action="bjtjMoveUp">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↑"></s:submit>
							</s:form>
							<s:form action="bjtjMoveDown">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="↓"></s:submit>
							</s:form>
						</td>
						<td width="120">
							<s:form action="bjtjBianji">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="编辑"></s:submit>
							</s:form>
						</td>
						<td width="60">
							<s:form action="bjtjShanchu">
								<s:hidden value="%{#als.id}" name="id"></s:hidden>
								<s:submit value="删除" onclick="return confirm('确认删除此条编辑推荐？')"></s:submit>
							</s:form>
						</td>
					</tr>
				</s:iterator>
					...
					<td colspan="8" height="26" align="center">
						...
						<s:bean name="org.apache.struts2.util.Counter" var="i">
							...
							<s:iterator>
								...
								<s:else>
									<a
										href="bjtjShowall?page=<s:property value='#i.current - 1' />">
										<s:property value="#i.current - 1" /> </a>&nbsp;
								</s:else>
							</s:iterator>
						</s:bean>
						<s:if test="#session.pageCount != page">
							<a href="bjtjXyy">下一页</a>
						</s:if>
					</td>
				</tr>
			</table>
